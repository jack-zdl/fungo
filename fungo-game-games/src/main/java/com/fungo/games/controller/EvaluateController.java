	...
	public ResultDto<String> delEvaluationDetail(MemberUserProfile memberUserPrefile,@RequestBody DelObjectListVO commentIds) {
		try {
			...
			if(CommonEnum.SUCCESS.code().equals(String.valueOf(resultDto.getStatus()))){
				...
				// 个人信息
				fungoCacheGame.excIndexCache(false, FungoCoreApiConstant.FUNGO_CORE_API_MEMBER_MINE_INFO + memberId, "", null);
				// fun币消耗详情
				fungoCacheGame.excIndexCache(false, FungoCoreApiConstant.FUNGO_CORE_API_MEMBER_MINE_INCENTS_FORTUNE_COIN_POST + memberId, "", null);
			}
			return resultDto;
		}catch (Exception e){
